...
static bool scope_import_unit(ident_t unit_name, lib_t lib, bool all,
  bool pqual, const loc_t *loc);
...
static tree_t sem_add_port(tree_t d, type_t type, port_mode_t mode,
  tree_t def);
...
static tree_t sem_builtin_fn(ident_t name, type_t result, const char *builtin,
  ...);
...
static bool sem_check_map(tree_t t, tree_t unit, tree_formals_t tree_Fs,
...
static bool sem_check_selected_name(ident_t name, tree_t where,
  tree_t *pdecl);
...
static bool sem_check_stmts(tree_t t, tree_t (*get_stmt)(tree_t,
  unsigned), int nstmts);
...
static bool sem_no_access_file_or_protected(tree_t t, type_t type,
  const char *what);
...
sem_ambiguous_cmp (
  ...
  return (sem_ambiguous_rate(tree_value(a)) -
         sem_ambiguous_rate(tree_value(b)));
} /* sem_ambiguous_cmp() */
...
sem_check_alias (
  ...
  if (type && type_is_subprogram(type)) {
    ...
    tree_t decl = NULL;
    if ((tree_kind(value) == T_REF) &&
      (decl = scope_find(tree_ident(value)))) {
    ...
    case ATTR_INSTANCE_NAME:
      {
        ...
        if ((class != C_SIGNAL) && (class != C_ARCHITECTURE) &&
          (class != C_ENTITY) &&
        ...
        case T_ALIAS:
          {
            if (tree_has_type(decl) &&
              (type_kind(tree_type(decl)) == T_FUNC)) {
...
sem_check_interface_class (
  ...
  if (tree_has_value(port)) {
    if (class == C_SIGNAL) {
      sem_error(port,
        "parameter of class SIGNAL can not have a default value");
    }

    if (class == C_VARIABLE) {
      if ((mode == PORT_OUT) || (mode == PORT_INOUT)) {
        sem_error(port,
          "parameter of class VARIABLE with mode OUT or INOUT can not have a default value");
      }
    }
    ...
    if (kind == T_PROTECTED) {
      sem_error(port,
        "parameter with protected type can not have a default value");
    }
  }
  ...
  if (sem_has_access(type) && (class != C_VARIABLE)) {
    sem_error(port,
      "object %s with type containing an access type must have class VARIABLE",
      istr(tree_ident(port)));
  }
...
static bool
sem_check_missing_subprogram_body (
  tree_t body,
  tree_t spec
) {
...
sem_check_pcall (
    ...
    if ((decl = scope_find_nth(name, n++))) {
      switch (tree_kind(decl))
      {
        ...
        case T_ALIAS:
          {
            if (tree_has_type(decl) &&
              (type_kind(tree_type(decl)) == T_PROC)) {
              decl = tree_ref(tree_value(decl));
              found_proc++;
              break;
            }
  ...
  const bool in_pure_func = in_func &&
    !(tree_flags(top_scope->subprog) & TREE_F_IMPURE);
...
sem_check_stmts (
  tree_t t,
  tree_t (*get_stmt)(tree_t, unsigned),
  int nstmts
) {
  bool ok = true;
...
static void
sem_check_subprogram_matches_spec (
  tree_t subprog,
  tree_t proto
) {
  ...
  switch (kind)
  {
    ...
    case T_FILE:
      // Rules for file types are in LRM 93 section 3.4
      {
        ...
        if (sem_has_access(f)) {
          sem_error(t, "type %s has a subelement with an access type", sem_type_str(
              f));
        }
...
static bool
sem_check_valid_implicit_signal (
  tree_t t
) {
    ...
    case T_UARRAY:
      {
        ...
        if (array_dimension(t) == 1) {
          ...
          sem_declare_binary(decl, ident_new("\"<=\""), t, t, std_bool,
            "aleq");
...
sem_locally_static (
  ...
  if (kind == T_REF) {
    ...
    if (dkind == T_CONST_DECL) {
      ...
      (relax_rules() & RELAX_LOCALLY_STATIC)) &&
      (dkind == T_PORT_DECL)) {
...
static bool
sem_no_access_file_or_protected (
  tree_t      t,
  type_t      type,
  const char *what
) {
  ...
  if (sem_has_access(type)) {
    sem_error(t, "%s may not have a type with a subelement of access type",
      what);
  }
