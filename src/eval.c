...
#define EVAL_ASSERT_VALUE(op, value, expect)         \
  do {                                               \
...
#define EVAL_ASSERT_VALID(op, value)                                    \
  do {                                                                  \
...
static bool eval_new_var(value_t *value, vcode_type_t type,
  eval_state_t *state);
...
static tree_t eval_value_to_tree(value_t *value, type_t type,
  const loc_t *loc);
...
eval (
  ...
  if (type_is_array(type)) {
    ...
    (tree_flags(tree_ref(expr)) & TREE_F_IMPURE)) {
    return (expr);
...
eval_find_unit (
  ...
  if (vcode == NULL) {
    ...
    if ((lib_name != unit_name) &&
      ((lib = lib_find(lib_name, false)) != NULL)) {
    ...
    case VCODE_TYPE_RECORD:
      {
        ...
        for (int i = 0; i < nfields; i++) {
          if (!eval_new_var(&(value->fields[i]), vtype_field(type, i),
            state)) {
            return (false);
          }
        }
    ...
    case VCODE_TYPE_RECORD:
      {
        if ((result->pointer = eval_alloc(sizeof(value_t), state))) {
          ...
          result->pointer->fields =
            eval_alloc(sizeof(value_t) * nfields, state);
        }
      }
      break;
    ...
    case VALUE_INTEGER:
      {
        if (low->integer > high->integer) {
          break;
        } else if ((reg->integer < low->integer) ||
          (reg->integer > high->integer)) {
          state->failed = true;
        }
      }
      break;
  ...
  if (vcode_count_args(op) == 1) {
    ...
    switch (map->image_map->kind)
    {
      case IMAGE_ENUM:
        {
          if ((object->integer < 0) ||
            (object->integer >= map->image_map->nelems)) {
