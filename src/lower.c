...
static void lower_case_add_branch(case_state_t *where, int left, int right,
  int depth, int dirmul, tree_t value,
  tree_t stmts);
...
static void lower_case_emit_state_code(case_state_t *state, vcode_reg_t value,
  int depth, vcode_block_t exit_bb,
...
static vcode_reg_t *lower_const_array_aggregate(tree_t t, type_t type, int dim,
  int *n_elems);
...
static bool lower_driver_nets(tree_t t, tree_t *decl, vcode_reg_t *all_nets,
  int64_t *all_length, vcode_reg_t *driven_nets,
  int64_t *driven_length, bool *has_non_const,
  tree_t proc);
...
static bool lower_memset_bit_pattern(tree_t value, unsigned bits,
  uint8_t *byte);
...
static vcode_reg_t lower_record_eq(vcode_reg_t r0, vcode_reg_t r1,
  type_t type);
...
lower_array_off (
  ...
  const bool wrapped = vtype_kind(vcode_reg_type(array)) ==
    VCODE_TYPE_UARRAY ||
...
static bool
lower_assign_can_use_storage_hint (
  tree_t stmt
) {
    ...
    case ATTR_RIGHT:
      {
        ...
        return (emit_cast(lower_type(tree_type(expr)), VCODE_INVALID_TYPE,
               reg));
      }
    ...
    case ATTR_VALUE:
      {
        ...
        return (emit_cast(lower_type(name_type), lower_bounds(name_type),
               reg));
      }
...
lower_builtin (
  ...
  if (icmp(builtin, "eq")) {
    ...
    return (emit_not(lower_array_cmp(r0, r1, r0_type, r1_type,
           VCODE_CMP_LEQ)));
...
static int64_t
lower_case_find_choice_element (
  tree_t value,
  int    depth
) {
...
static void
lower_cleanup_temp_objects (
  vcode_reg_t saved_heap
) {
  if (tmp_alloc_used) {
    ...
    case T_CONST_DECL:
      {
        if (type_is_scalar(tree_type(decl)) ||
          tree_attr_int(decl, deferred_i, 0)) {
          break;
        }
        // Fall-through
      }
...
lower_finished (
  ...
  if (verbose != NULL) {
    if ((*verbose == '\0') ||
      (strstr(istr(vcode_unit_name()), verbose) != NULL)) {
      vcode_dump();
    }
  }
} /* lower_finished() */
...
static bool
lower_scalar_has_static_bounds (
  type_t       type,
  vcode_reg_t *low_reg,
  vcode_reg_t *high_reg
) {
...
static void
lower_subprogram_ports (
  tree_t body,
  bool   has_subprograms
) {
...
static bounds_kind_t
lower_type_bounds_kind (
  type_t type
) {
...
lower_var_ref (
  ...
  if (var == VCODE_INVALID_VAR) {
    ...
    return (emit_index(var, VCODE_INVALID_REG));
  } else if ((type_is_scalar(type) || type_is_file(type) ||
    type_is_access(type)) &&
    (ctx == EXPR_LVALUE)) {
    return (emit_index(var, VCODE_INVALID_REG));
  } else {
