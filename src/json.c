...
static JsonNode *dump_params(tree_t t, get_fn_t get, int n,
  const char *prefix);
...
dump_arch (
  ...
  json_append_member(architecture_node, "name",
    json_mkstring(istr(tree_ident(t))));
  json_append_member(architecture_node, "of",
    json_mkstring(istr(tree_ident2(t))));
      ...
      case T_LIBRARY:
        {
          if ((tree_ident(c) != std_i) && (tree_ident(c) != work_i)) {
            ...
            json_append_member(library, "name",
              json_mkstring(istr(tree_ident(c))));
      ...
      case T_USE:
        {
          ...
          if (tree_has_ident2(c)) {
            json_append_member(use, "elt",
              json_mkstring(istr(tree_ident2(c))));
          } else {
    ...
    case T_TYPE_DECL:
      {
        ...
        if (is_subtype) {
          json_append_member(decl, "subtype_name",
            json_mkstring(istr(type_ident(type_base(type)))));
        }
        ...
        if (type_is_integer(type) || type_is_real(type)) {
          ...
          JsonNode *units = json_mkarray();
          {
            ...
            for (int i = 0; i < nunits; i++) {
              ...
              json_append_member(unit, "type",
                json_mkstring(istr(tree_ident(u))));
          ...
          for (unsigned i = 0; i < type_enum_literals(type); i++) {
            json_append_element(enum_val,
              json_mkstring(istr(tree_ident(type_enum_literal(type, i)))));
          }
    ...
    case T_ATTR_SPEC:
      {
        ...
        json_append_member(decl, "class",
          json_mkstring(class_str(tree_class(t))));
        json_append_member(decl, "value", dump_expr(tree_value(t)));
        return (decl);
      }
    ...
    case T_FUNC_DECL:
      {
        ...
        json_append_member(decl, "ret_type",
          json_mkstring(type_pp(type_result(tree_type(t)))));
        return (decl);
      }
    ...
    case T_FCALL:
      {
        ...
        json_append_member(expr_node, "name",
          json_mkstring(istr(tree_ident(tree_ref(t)))));
        json_append_member(expr_node, "params",
    ...
    case T_LITERAL:
      {
        switch (tree_subkind(t))
        {
          case L_INT:
            {
              ...
              json_append_member(expr_node, "value", json_mknumber(tree_ival(
                  t)));
            }
            break;
          ...
          case L_REAL:
            {
              ...
              json_append_member(expr_node, "value", json_mknumber(tree_dval(
                  t)));
            }
            break;
          ...
          case L_STRING:
            {
              if (type_ident(tree_type(t)) ==
                ident_new("STD.STANDARD.STRING")) {
                json_append_member(expr_node, "cls", json_mkstring("string"));
              } else {
                json_append_member(expr_node, "cls", json_mkstring(
                    "string_lit"));
              }
              {
    ...
    case T_AGGREGATE:
      {
        ...
        for (unsigned i = 0; i < tree_assocs(t); i++) {
          ...
          switch (tree_subkind(a))
          {
            case A_POS:
              {
                json_append_member(aggreg, "cls", json_mkstring(
                    "aggregate_pos"));
            ...
            case A_NAMED:
              {
                json_append_member(aggreg, "cls",
                  json_mkstring("aggregate_named"));
            ...
            case A_OTHERS:
              {
                json_append_member(aggreg, "cls",
                  json_mkstring("aggregate_others"));
                json_append_member(aggreg, "expr", dump_expr(value));
              }
              break;
            ...
            case A_RANGE:
              {
                json_append_member(aggreg, "cls",
                  json_mkstring("aggregate_range"));
                json_append_member(aggreg, "range", dump_range(tree_range(a,
                  0)));
                json_append_member(aggreg, "expr", dump_expr(value));
              }
              break;
    ...
    case T_ATTR_REF:
      {
        ...
        json_append_member(expr_node, "name", json_mkstring(istr(tree_ident(
            t))));
    ...
    case T_RECORD_REF:
      {
        ...
        json_append_member(expr_node, "of", dump_expr(tree_value(t)));
        json_append_member(expr_node, "item", json_mkstring(istr(tree_ident(
            t))));
      }
      break;
    ...
    case T_TYPE_CONV:
      {
        ...
        json_append_member(expr_node, "type",
          json_mkstring(istr(tree_ident(tree_ref(t)))));
        json_append_member(expr_node, "expr",
    ...
    case T_QUALIFIED:
      {
        ...
        json_append_member(expr_node, "type",
          json_mkstring(istr(type_ident(tree_type(t)))));
        json_append_member(expr_node, "expr", dump_expr(tree_value(t)));
      }
      break;
    ...
    case T_EXIT:
      {
        ...
        json_append_member(statement, "name",
          json_mkstring(istr(tree_ident2(t))));
    ...
    case T_PCALL:
      {
        ...
        json_append_member(statement, "name",
          json_mkstring(istr(tree_ident(tree_ref(t)))));
        json_append_member(statement, "params",
          dump_params(t, tree_param, tree_params(t), NULL));
      }
      break;
    ...
    case T_INSTANCE:
      {
        ...
        switch (tree_class(t))
        {
          case C_ENTITY:
            {
              json_append_member(statement, "inst_type", json_mkstring(
                  "entity"));
            }
            break;
          ...
          case C_COMPONENT:
            {
              json_append_member(statement, "inst_type",
                json_mkstring("component"));
            }
            break;
        ...
        json_append_member(statement, "name",
          json_mkstring(istr(tree_ident2(t))));
        json_append_member(statement, "iname",
          json_mkstring(istr(tree_ident(t))));
        if (tree_has_spec(t)) {
          tree_t bind = tree_value(tree_spec(t));
          json_append_member(statement, "bound_to",
            json_mkstring(istr(tree_ident(bind))));
          if (tree_has_ident2(bind)) {
            json_append_member(statement, "bound_from",
              json_mkstring(istr(tree_ident2(bind))));
          } else {
        ...
        if (tree_genmaps(t) > 0) {
          json_append_member(statement, "generic",
            dump_params(t, tree_genmap, tree_genmaps(t), ""));
        }

        if (tree_params(t) > 0) {
          json_append_member(statement, "port",
            dump_params(t, tree_param, tree_params(t), ""));
        }
      }
      break;
