...
static inline vcode_reg_t *vcode_reg_array_nth_ptr(vcode_reg_array_t *a,
  unsigned                                                            n);
static inline vcode_reg_t vcode_reg_array_nth(vcode_reg_array_t *a,
  unsigned                                                       n);
static inline vcode_block_t *vcode_block_array_nth_ptr(vcode_block_array_t *a,
  unsigned                                                                  n);
static inline vcode_block_t vcode_block_array_nth(vcode_block_array_t *a,
  unsigned                                                             n);
static inline vcode_type_t *vcode_type_array_nth_ptr(vcode_type_array_t *a,
  unsigned                                                               n);
static inline vcode_type_t vcode_type_array_nth(vcode_type_array_t *a,
  unsigned                                                          n);
static inline op_t *op_array_nth_ptr(op_array_t *a, unsigned n);
static inline op_t op_array_nth(op_array_t *a, unsigned n);
static inline param_t *param_array_nth_ptr(param_array_t *a, unsigned n);
static inline param_t param_array_nth(param_array_t *a, unsigned n);
static inline var_t *var_array_nth_ptr(var_array_t *a, unsigned n);
static inline var_t var_array_nth(var_array_t *a, unsigned n);
static inline reg_t *reg_array_nth_ptr(reg_array_t *a, unsigned n);
static inline reg_t reg_array_nth(reg_array_t *a, unsigned n);
static inline block_t *block_array_nth_ptr(block_array_t *a, unsigned n);
static inline block_t block_array_nth(block_array_t *a, unsigned n);
static inline signal_t *signal_array_nth_ptr(signal_array_t *a, unsigned n);
static inline signal_t signal_array_nth(signal_array_t *a, unsigned n);
static inline vtype_t *vtype_array_nth_ptr(vtype_array_t *a, unsigned n);
static inline vtype_t vtype_array_nth(vtype_array_t *a, unsigned n);
static vcode_reg_t emit_arith(vcode_op_t kind, vcode_reg_t lhs,
  vcode_reg_t rhs);
static vcode_reg_t emit_fcall_op(vcode_op_t op, ident_t func,
  vcode_type_t type, const vcode_reg_t *args,
...
void
emit_alloc_driver (
  vcode_reg_t all_nets,
  vcode_reg_t all_length,
  vcode_reg_t driven_nets,
  vcode_reg_t driven_length,
  vcode_reg_t init
) {
...
emit_case (
  vcode_reg_t          value,
  vcode_block_t        def,
  const vcode_reg_t   *cases,
  const vcode_block_t *blocks,
  int                  ncases
) {
...
emit_null_check (
  ...
  VCODE_FOR_EACH_OP(other)
  {
    if ((other->kind == VCODE_OP_NULL_CHECK) &&
      (other->args.items[0] == ptr)) {
      return;
...
emit_store (
  ...
  VCODE_FOR_EACH_OP(other)
  {
    ...
    } else if (((other->kind == VCODE_OP_INDEX) ||
      (other->kind == VCODE_OP_LOAD)) &&
      (other->address == var)) {
...
emit_uarray_len (
  ...
  VCODE_FOR_EACH_OP(other)
  {
    ...
    } else if ((other->kind == VCODE_OP_WRAP) &&
      (other->result == array)) {
...
emit_unwrap (
  ...
  VCODE_FOR_EACH_OP(other)
  {
    if ((other->kind == VCODE_OP_WRAP) && (other->result == array)) {
      return (other->args.items[0]);
    } else if ((other->kind == VCODE_OP_UNWRAP) &&
      (other->args.items[0] == array)) {
      return (other->result);
    }
  }
...
const vcode_res_fn_t *
vcode_get_resolution (
  int op
) {
  op_t *o = vcode_op_data(op);
    ...
    case VCODE_OP_LOAD:
      {
        ...
        for (int i = 0; i < active_unit->blocks.count; i++) {
          ...
          for (int j = 0; j < b->ops.count; j++) {
            ...
            if ((op->kind == VCODE_OP_STORE) &&
              (op->address == defn->address)) {
...
vcode_op_string (
  ...
  static const char *strs[] =
  {
    ...
    "not",
    "and",              "nested fcall",
    "param upref",      "resolved address", "set initial",
    "alloc driver",
    "event",            "active",           "const record",
    "record ref",
    "copy",             "sched event",
    "pcall",            "resume",           "memcmp",
    "xor",
    "xnor",             "nand",             "nor",
    "memset",
    "vec load",         "case",             "endfile",
    "file open",
    "file write",       "file close",
    "file read",        "null",             "new",
    "null check",
    "deallocate",       "all",
    "bit vec op",       "const real",       "value",
    "last event",
    "needs last value",
...
vtype_named_record (
  ident_t             name,
  const vcode_type_t *field_types,
  int                 nfields
) {
  assert(active_unit != NULL);
...
emit_logical (
  ...
  if (l_is_const && r_is_const) {
    switch (op)
    {
      case VCODE_OP_AND:
        {
          return (emit_const(vtbool, lconst && rconst));
        }
...
emit_logical_identity (
  ...
  switch (op)
  {
    case VCODE_OP_AND:
      {
        return (b ? reg : emit_const(vtype_bool(), 0));
      }

    case VCODE_OP_OR:
      {
        return (b ? emit_const(vtype_bool(), 1) : reg);
      }

    case VCODE_OP_XOR:
      {
        return (b ? emit_not(reg) : reg);
      }

    case VCODE_OP_XNOR:
      {
        return (b ? reg : emit_not(reg));
      }

    case VCODE_OP_NAND:
      {
        return (b ? emit_not(reg) : emit_const(vtype_bool(), 1));
      }

    case VCODE_OP_NOR:
      {
        return (b ? emit_const(vtype_bool(), 0) : emit_not(reg));
      }
...
emit_uarray_op (
  ...
  VCODE_FOR_EACH_OP(other)
  {
    if ((other->kind == o) && (other->args.items[0] == array) &&
      (other->dim == dim) &&
