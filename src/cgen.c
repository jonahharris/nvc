...
static inline size_list_t *size_list_array_nth_ptr(size_list_array_t *a,
  unsigned                                                            n);
static inline size_list_t size_list_array_nth(size_list_array_t *a,
  unsigned                                                       n);
static void cgen_add_func_attr(LLVMValueRef fn, func_attr_t attr, int param);
static void cgen_alloc_context(cgen_ctx_t *ctx);
static void cgen_append_size_list(size_list_array_t *list, vcode_type_t elem,
  unsigned count,
...
static _Bool cgen_is_uarray_struct(LLVMValueRef meta);
...
static void cgen_op_fcall(int op, _Bool nested, cgen_ctx_t *ctx);
...
static void cgen_op_pcall(int op, _Bool nested, cgen_ctx_t *ctx);
...
static void cgen_op_resume(int op, _Bool nested, cgen_ctx_t *ctx);
...
static LLVMValueRef llvm_int1(_Bool b);
...
static void
cgen_append_size_list (
  size_list_array_t    *list,
  vcode_type_t          elem,
  unsigned              count,
  const vcode_res_fn_t *resolution,
  size_t               *res_elem
...
cgen_op_cmp (
  ...
  if (arg_kind == VCODE_TYPE_REAL) {
    ...
    switch (vcode_get_cmp(op))
    {
      ...
      case VCODE_CMP_LT:
        {
          pred = is_signed ? LLVMIntSLT : LLVMIntULT;
        }
        break;

      case VCODE_CMP_GT:
        {
          pred = is_signed ? LLVMIntSGT : LLVMIntUGT;
        }
        break;

      case VCODE_CMP_LEQ:
        {
          pred = is_signed ? LLVMIntSLE : LLVMIntULE;
        }
        break;

      case VCODE_CMP_GEQ:
        {
          pred = is_signed ? LLVMIntSGE : LLVMIntUGE;
        }
        break;
    }
...
static LLVMValueRef
cgen_resolution_wrapper (
  const vcode_res_elem_t *rdata
) {
...
cgen_type (
  ...
  switch (vtype_kind(type))
  {
    case VCODE_TYPE_INT:
      {
        return (LLVMIntType(bits_for_range(vtype_low(type),
               vtype_high(type))));
      }
...
llvm_uarray_type (
  ...
  switch (LLVMGetTypeKind(base))
  {
    case LLVMIntegerTypeKind:
      {
        struct_name = xasprintf("uarray.i%d.%d", LLVMGetIntTypeWidth(
              base), dims);
      }
      break;
